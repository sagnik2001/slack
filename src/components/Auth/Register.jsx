...
const Register=()=>{
  ...
  const isFormEmpty=()=>{
    return !User.username.length||!User.password.length||!User.email.length||!User.confirmPassword.length
  }
  const checkPassword=()=>{
    if(User.password.length < 6){
        setError((error)=>error.concat({message:"Given Password must be at least 6 characters"}))
      return false;
    }
  ...
  const saveData=(createUser)=>{
      SetLoading(true)
    userDatabaseCollection.child(createUser.user.uid).set({
  ...
  return(
    <Grid verticalalign="middle" textAlign="center" className="register" >
       <Grid.Column style={mystyle}>
         ...
         <Message>
           Already Have An Account ? <Link to="/login" >&nbsp;<Icon name="hand point right"/>LogIn</Link>
         </Message>
       </Grid.Column>
    </Grid>
  )
